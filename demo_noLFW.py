...
def approximate_rank_order_clustering(vectors, n_neighbors=200, thresh=1.1):
    ...
    """
    print n_neighbors,
    clusters = cluster(vectors, n_neighbors=n_neighbors, thresh= [thresh])
    return clusters
...
def rankOrder_cluster_format(vectors, n_neighbors=20, thresh=0.3):
    clusters_thresholds = approximate_rank_order_clustering(vectors, n_neighbors=n_neighbors, thresh=thresh)
    clusters_at_th = clusters_thresholds[0]
    y_pred = np.zeros(vectors.shape[0])
